...
export default function BetSetup({ players, onConfirm, onCancel }: BetSetupProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center px-4">
      <div className="bg-gray-900 rounded-2xl border border-gray-700 p-5 max-w-sm w-full max-h-[90vh] overflow-y-auto">
        <h3 className="text-lg font-bold text-green-400 mb-4">Set Up a Bet</h3>
        ...
        <div className="relative mb-3">
          <select
            ...
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-gray-200 focus:outline-none focus:border-green-500 appearance-none"
          >
            {BET_TYPE_LIST.map(t => (
        ...
        {showExplainer && (
          <div className="bg-gray-800 rounded-lg p-3 mb-3 border border-gray-700">
            <p className="text-sm text-gray-300 mb-2">{selectedType.description}</p>
            <div className="space-y-1">
              {selectedType.howToPlay.map((step, i) => (
                <p key={i} className="text-xs text-gray-400">
                  <span className="text-green-400 font-medium">{i + 1}.</span> {step}
                </p>
              ))}
            </div>
            <p className="text-xs text-gray-500 mt-2 pt-2 border-t border-gray-700">
              <span className="text-gray-400 font-medium">Settlement:</span> {selectedType.settlement}
            </p>
          </div>
        )}
        ...
        <input
          ...
          className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-green-500 mb-3"
        />
        ...
        {selectedType.usesHandicap && (
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm text-gray-400">Use handicap strokes</span>
            <button
              onClick={() => setUseHandicap(!useHandicap)}
              className={`w-12 h-6 rounded-full transition-colors relative ${
                useHandicap ? 'bg-green-600' : 'bg-gray-700'
              }`}
            >
        ...
        {selectedType.supportsPress && (
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm text-gray-400">Auto-press when down by</span>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 bg-green-600 hover:bg-green-500 disabled:bg-gray-700 disabled:text-gray-500 text-white py-2.5 rounded-xl font-medium transition-colors"
          >
            Lock It In
          </button>
        </div>
